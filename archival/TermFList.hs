...
pretty t = pretty' (emptyMb t) ctxtFListNil 0
  where pretty' :: Mb ctx (Term a) -> CtxtFList StringF ctx -> Int -> String
        pretty' u varnames n =
          case mbTerm u of
            BVar b -> {-# SCC "cas1" #-} case mbCmpName b of
                        Left pf  -> {-# SCC "cas1.1" #-} unStringF (lookupCtxtFList pf varnames)
                        Right _ ->  {-# SCC "cas1.2" #-} "free"
            BLam b -> {-# SCC "cas2" #-} let x = showVar n in
                       {-# SCC "cas2.1" #-} "(\\" ++ x ++ "." ++ ({-# SCC "cas2.1.1" #-} pretty' (combineMb b) (varnames |> (StringF x)) (n+1)) ++ ")"
            BApp b1 b2 -> {-# SCC "cas3" #-} "(" ++ pretty' b1 varnames n ++ " " ++ pretty' b2 varnames n ++ ")"
...
-- to make a function for CtxtFList
...
msubst ts b = 
    case mbTerm b of
      ...
      BVar bx -> case mbCmpName bx of
                   Left p  -> return $ lookupCtxtFList p ts
                   Right i -> var i
    ...
    case (mbTerm b1, mbTerm b2) of
      (BVar bv1, BVar bv2)         ->
          case (mbCmpName bv1, mbCmpName bv2) of 
            (Left p1, Left p2)   -> cmpInCtxt p1 p2
...
closeTerm b ns = 
    case mbTerm b of
      ...
      (BVar bx)    -> case mbCmpName bx of
                        Left p  -> var $ lookupCtxtFList p ns
                        Right i -> var i
    ...
    case mbTerm b of
      ...
      BApp b1 b2
        | BLam b11 <- mbTerm b1 -> do
            t2 <- closeTerm b2 ns
            Just <$> msubst (ctxtFListMap Var ns |> t2) (combineMb b11)
        | otherwise -> do
            mt1 <- norm1' b1 ns
            t2  <- closeTerm b2 ns
            return $ case mt1 of 
...
norm1 t = norm1' (emptyMb t) ctxtFListNil
