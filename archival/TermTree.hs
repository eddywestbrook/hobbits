...
pretty t = pretty' (emptyMb t) CtxtFTreeNil 0
  where pretty' :: Mb ctx (Term a) -> CtxtFTree StringF ctx -> Int -> String
        pretty' u varnames n =
          case mbTerm u of
            BVar b -> case mbCmpName b of
                        Left pf  -> unStringF (lookupCtxFTree pf varnames)
                        Right _ -> "free"
            BLam (b :: Mb ctx (Binding a (Term b))) ->
                let x = showVar n in
                "(\\" ++ x ++ "."
                      ++ pretty' (combineMb b)
                                 (varnames |> ((StringF x) :: StringF a)) (n+1)
                      ++ ")"
...
-- to make a function for CtxtFTree
...
-- alpha equivalence
samePath :: InTree ctx1 a1 -> InTree ctx2 a2 -> Bool
samePath InTreeBase InTreeBase           = True
samePath (InTreeL p1) (InTreeL p2)       = samePath p1 p2
samePath (InTreeR p1) (InTreeR p2)       = samePath p1 p2
samePath _               _               = False
    ...
    case (mbTerm b1, mbTerm b2) of
      (BVar bv1, BVar bv2)         -> 
          case (mbCmpName bv1, mbCmpName bv2) of 
            (Left p1, Left p2)     -> samePath p1 p2
            (Right n1, Right n2)   -> maybeToBool (cmpName n1 n2)
            _                      -> False
...
closeTerm :: Mb ctx (Term a) -> CtxtFTree Name ctx -> Fresh (Term a)
closeTerm b ns = 
    case mbTerm b of
      (BApp b1 b2) -> app (closeTerm b1 ns) (closeTerm b2 ns)
      (BLam b1)    -> lam $ \n -> closeTerm (combineMb b1) (ns |> n)
      (BVar bx)    -> case mbCmpName bx of
                        Left p  -> var $ lookupCtxFTree p ns
                        Right i -> var i

convertCtx :: CtxtFTree Name ctx -> CtxtFTree Term ctx
convertCtx names = ctxtFTreeMap Var names


-- cbn normalizer under bindings
norm1' :: Mb ctx (Term a) -> CtxtFTree Name ctx -> Fresh (Maybe (Term a))
norm1' b ns = 
    case mbTerm b of
...
norm1 t = norm1' (emptyMb t) CtxtFTreeNil
...
-- matching ParseResults under binders
